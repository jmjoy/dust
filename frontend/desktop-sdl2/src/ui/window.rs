...
use {
    sdl2::event::Event,
    sdl2::video::Window as Sdl2Window,
};
...
impl GfxSurface {
    fn new(window: &Sdl2Window, gfx: &GfxDevice, srgb_mode: SrgbMode) -> Self {
        let surface = unsafe {
...
impl Builder {
    pub async fn new(
        ...
        let window = video_subsystem
            .window("dust-desktop-sdl2", 1080, 720)
            .position_centered()
            .resizable()
            .build()
            .unwrap();
    ...
    pub fn run<S: 'static>(
        ...
        loop {
            let event = self.event_loop.poll_event();
            // for event in self.event_loop.poll_event() {
                ...
                // if let Event::Window { win_event: WindowEvent::Shown, .. } = &event {
            ...
            if let Some(event) = &event {
                ...
                let mut redraw = || {
                    ...
                    let frame = window
                        .gfx_surface
                        .start_frame(&window.gfx_device, inner_size);
                    ...
                    window
                        .imgui_sdl2
                        .prepare_frame(imgui, &window.window, &self.event_loop);
                    ...
                    window.imgui_sdl2.prepare_frame(imgui, &window.window, &self.event_loop);
